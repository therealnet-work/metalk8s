...
Feature: Cluster expansion
    ...
    Scenario: ETCD failover on multi-member
        Given the Kubernetes API is available
        ...
        When we remove "node2" from the etcd cluster
        Then we have 1 running pod labeled 'component=kube-apiserver' in namespace 'kube-system' on node 'node2'
